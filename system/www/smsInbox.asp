...
function initial(){
	...
	//setTimeout("update_smsIn();", 5000);

	bindingEvents(); 
}
function update_smsIn(e) {
    $j.ajax({
        url: '/sms_in.asp',
        dataType: 'script',
        error: function(xhr) {
...
deleteSelectedSimMsgClickHandler = function() {
    ...
    if (confirm("<#confirm_sms_delete#>")) {
        ...
        for (var i = 0; i < msgIds.length; i++) {
	     ...
	     document.form.current_page.value = "smsInbox.asp";
	     document.form.next_page.value = "smsInbox.asp";
	     document.form.box_type.value = "0";//删除类型 ：发送或者收件箱 
...
function check_full_scan_done() {
    if (nmap_fullscan != "1") {
        ...
        setTimeout("update_smsIn();", 2000);
    }
}

var resultMessages = [];
function getAllSMS() {
        resultMessages = parseMessages(smsIn);
...
function enableBtn(ele) {
    ...
    /**
    * 添加分页数据
    * @param sltID：为标签<select id=sltID>的ID
    */
                ...
                <div class="row-fluid">
                    <div class="span12">
                        <div class="box well grad_colour_dark_blue">
                            <h2 class="box_head round_top"><#menu11#> - <#menu5_11_1#></h2>
                                        ...
                                        <div class="simMsgList-btns smslist-btns">
                                                    <div class="row-fluid" >
                                                             <div class="sms-operator">
                                                                  <!--<span id="boxmessage"style="margin-left: 20px;"><#Inbox#></span>-->
                                                                   <div>
                                                                        <input id="simMsgList-delete" type="button" class="common-btn" value="<#CTL_del#>" onclick="deleteSelectedSimMsgClickHandler();" style="margin-left: 500px;">
                                                                 </div>
                                                               </div>
