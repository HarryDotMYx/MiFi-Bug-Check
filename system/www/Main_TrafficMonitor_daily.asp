...
function getYMD(n){
	return [((n >> 16) & 0xFFFF), ((n >> 8) & 0xFF), (n & 0xFF)];
}
...
function makeRow(rtitle, dl, ul, total,time,cname){
	return '<tr>' +
		'<td class="rtitle">' + rtitle + '</td>' +
		'<td class="dl">' + dl + '</td>' +
		'<td class="ul">' + ul + '</td>' +
		'<td class="total">' + total + '</td>' +
		'<td class="clean"> <input class="btn-primary '+ cname + '"'+ ' id="' + time + '"' + ' onclick="clean_data(this);" type="button" value="<#Clean#>" /></td>'
		'</tr>';
}
...
function redraw_history(){
	...
	if(hash == '#dy'){
		...
		grid += '<tr><th width="20%" valign="top" style="text-align:left"><#Date#></th>';
		...
		grid += '<th width="20%" style="text-align:right" valign="top"><#Total#></th>';
		grid += '<th width="20%" style="text-align:right" valign="top"><#Clean#></th></tr>';
	}
	...
	if (hash == '#dy'){
		...
		dafm_option = "";
		for(i = 0; i < month_times.length; i++){
			ymd = getYMD(month_times[i]);
			dafm_option += '<option value=' + month_times[i] + '>' + ymText(ymd[0], ymd[1]) + '</option>';
		}
		E('dafm').innerHTML = dafm_option;
		
		for (i = 0; i < daily_history.length; ++i){
			h = daily_history[i];
			ymd = getYMD(h[0]);
			grid += makeRow(ymdText(ymd[0], ymd[1], ymd[2]), rescale(h[1]), rescale(h[2]), rescale(h[1] + h[2]),h[0],"dclean");
		}
	}else{
		...
		for (i = 0; i < monthly_history.length; ++i){
			h = monthly_history[i];
			ymd = getYMD(h[0]);
			grid += makeRow(ymText(ymd[0], ymd[1]), rescale(h[1]), rescale(h[2]), rescale(h[1] + h[2]),h[0],"mclean");
		}
	}
	E('bwm-daily-grid').innerHTML = grid + '</table>' + '<p style="text-align: center;"><#Alert_Traffic#></p>';
}
...
.table td.dl, .table td.ul, .table td.total,.table td.clean {text-align: right;}
</style>
</head>
...
<div class="wrapper">
    ...
    <input type="hidden" name="current_page" value="Main_TrafficMonitor_daily.asp#DY">
	...
	<input type="hidden" name="clean_id" value="">
                        ...
                        <div class="box well grad_colour_dark_blue">
                            ...
                            <div class="round_bottom">
                                <div class="row-fluid">
                                    ...
                                    <table width="100%" cellpadding="4" cellspacing="0" class="table table-striped" style="margin-bottom: 0px;">
                                        <tr>
                                            <td width="50%" style="border-top: 0 none;">
                                                <#Date#>:&nbsp;
                                                <select name="dafm" id="dafm" style="width: 200px;" onchange="update_daily_data(this)">
                                    ...
                                    <div id="bwm-daily-grid" style="max-height:480px;overflow-y:auto;"></div>
