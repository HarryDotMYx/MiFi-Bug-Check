...
function setLogData() {
    ...
    $j.get('/log_content.asp', function(data) {
        ...
        if ($j.browser.msie && !is_ie11p)
            data = data.nl2br();
        if ($j("#textarea").val() == '') {
            $j("#textarea").text(data);
           $j("#textarea").prop('scrollTop', $j("#log_area").prop('scrollHeight'));
            $j("#scrATop").val($j("#textarea").prop('scrollTop'));
        } else {
            var scrMaxTop = $j("#textarea").prop('scrollHeight')
            var scrTop = $j("#textarea").prop('scrollTop');
            $j("#textarea").text(data);
            var scrITop = scrMaxTop - scrTop;
            if ($j("#scrATop").val() == scrTop || scrITop < 629) {
                $j("#textarea").prop('scrollTop', scrMaxTop);
                $j("#scrATop").val($j("#textarea").prop('scrollTop'));
            } else {
                $j("#textarea").prop('scrollTop', scrTop);
            }
        }
    });
}
                ...
                <div class="row-fluid">
                    <div class="span12">
                        <div class="box well grad_colour_dark_blue">
                            <h2 class="box_head round_top"><#menu5_8#> - <#menu5_7_2#></h2>
