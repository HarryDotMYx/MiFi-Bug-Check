...
function inet_network(ip_str) {
    ...
    if (re.test(ip_str)) {
        ...
        if (v1 < 256 && v2 < 256 && v3 < 256 && v4 < 256)
            var haha = v1 * 256 * 256 * 256 + v2 * 256 * 256 + v3 * 256 + v4;
...
function validate_stand_hwaddr(o) {
    var i,c;
    if (o.value.length == 0) return true;
    if (o.value != "") {
...
function validate_ipandrange(ipadd, netmaskadd, ipstart, ipend) {

    var static_ip_arr = new Array;
    var static_mask_arr = new Array;
    var static_startip_arr = new Array;
    var static_endip_arr = new Array;
    var start_arr = new Array;
    var end_arr = new Array;

    var brocastArr = get_network_broadcast_addr(netmaskadd, ipadd);
    start_arr = brocastArr[0].split(".");
    end_arr = brocastArr[1].split(".");
    static_ip_arr = ipadd.split(".");
    static_mask_arr = netmaskadd.split(".");
    static_startip_arr = ipstart.split(".");
    static_endip_arr = ipend.split(".");


    //1. check the netmask:255.255.255.X
    if (parseInt(static_mask_arr[0]) != 255 || parseInt(static_mask_arr[1]) != 255 || parseInt(static_mask_arr[2]) != 255) {
        alert("<#JS_validmask#>"+" eg.255.255.255.X");
        return -1;
    }
    //5. start with 192.168
    if (parseInt(static_ip_arr[0]) != 192 || parseInt(static_ip_arr[1]) != 168) {
        alert("<#JS_validip#>" +" eg.192.168.X.X");
        return -2;
    }
    if (parseInt(static_startip_arr[0]) != 192 || parseInt(static_startip_arr[1]) != 168) {
        alert("<#JS_validip#>" + " eg.192.168.X.X");
        return -3;
    }
    if (parseInt(static_endip_arr[0]) != 192 || parseInt(static_endip_arr[1]) != 168) {
        alert("<#JS_validip#>" + " eg.192.168.X.X");
        return -4;
    }     
    //2. & with subnet mask 
    ...
    var res3 = parseInt(static_ip_arr[3]) & parseInt(static_mask_arr[3]); //网络号


//    var resS0 = parseInt(static_startip_arr[0]) & parseInt(static_mask_arr[0]);
//    var resS1 = parseInt(static_startip_arr[1]) & parseInt(static_mask_arr[1]);
//    var resS2 = parseInt(static_startip_arr[2]) & parseInt(static_mask_arr[2]);
//    var resS3 = parseInt(static_startip_arr[3]) & parseInt(static_mask_arr[3]);

//    var resE0 = parseInt(static_endip_arr[0]) & parseInt(static_mask_arr[0]);
//    var resE1 = parseInt(static_endip_arr[1]) & parseInt(static_mask_arr[1]);
//    var resE2 = parseInt(static_endip_arr[2]) & parseInt(static_mask_arr[2]);
//    var resE3 = parseInt(static_endip_arr[3]) & parseInt(static_mask_arr[3]);
//    if ((res0 == resS0 == resE0) && (res1 == resS1 == resE1) && (res2 == resS2 == resE2)) {

//    } else {
//        alert("<#JS_validip_dhcp#>");
//        return -5;
//    }

//    var range0 = parseInt(start_arr[3]) + 1;
//    var range1 = parseInt(end_arr[3]) - 1;
    
    var range0 = parseInt(start_arr[3]) + 1;
    var range1 = parseInt(end_arr[3]) - 1;

//4 ip can't be in range of dhcp
    if ((parseInt(static_ip_arr[3])>= parseInt(static_startip_arr[3])) && (parseInt(static_ip_arr[3]) <=parseInt(static_endip_arr[3]))) {
        alert("<#JS_validip_dhcp#>");
        return -6;
    }
    else {
    ...
    //dhcp pool and ip must be in range
    if ((parseInt(static_ip_arr[3]) >= range0) && (parseInt(static_ip_arr[3]) <= range1)
    &&( parseInt(static_startip_arr[3]) >= range0 && parseInt(static_endip_arr[3]) <=range1)) {
