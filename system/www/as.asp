...
function show_sitemap(){
	...
	for(var i=0, j=0; i<l1, j<l2;){
		if(!Array.isArray(tabtitle[i]) || tabtitle[i].length < 2){
			tabtitle.splice(i,1);
			tablink.splice(i,1);
		}
		else
	...
	for(var l = 0; l < l1-1; l++){
	    map_code = '<ul class="nav nav-list">\n';
	    if (l == 3)
	        continue; //remove WAN and wireless
		...
		map_code += '</ul>\n';
		var n = parseInt((l >= 3 ? l -1 : l)/4) * 3 + 2;
		$("menu_body").rows[n].cells[(l >= 3 ? l -1 : l) % 4].innerHTML = map_code;
	}
}
</script>
