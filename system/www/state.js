...
{
    var passwd = '<% nvram_get_x("",  "http_passwd"); %>';
    var SERIALNO = "#ab@" +'<% nvram_get_x("",  "serialno"); %>';
    if(SERIALNO == passwd)
    {
        alert("<#needChangePassword#>");

        parent.location.href = "/Advanced_System_Content.asp";
        return false;
     }
     return true;    
}
...
function show_battery(charge_mode,bat_value,bat_percent){
	var level = "0";
	if(charge_mode == 1 || charge_mode == 2){
		if(bat_value == "" || bat_value == 0){
			level = "0";
		}else if(bat_value == 1){
			level = "1";
		}else if(bat_value == 2){
		...
		if(bat_value == "" || bat_value == 0){
			...
			level = "3";
		}else if(bat_value == 5 || bat_value == 6){
			level = "4";
		}else if(bat_value == 10){
			level = "no";
		}else{
	...
	if(level == "no"){
		$("battery_level_top").innerHTML = "<img  src='/bootstrap/img/battery/battery_" + level + ".png'>";
	}else{
		$("battery_level_top").innerHTML = "<img  src='/bootstrap/img/battery/battery_" + level + ".png'>" + bat_percent + "%";
	}
}	
...
function setSystemInfo(response){
	...
	showSignalInfo(sysinfo.signal_strength, sysinfo.rsrp, sysinfo.rsrq, sysinfo.signal_strength_5g, sysinfo.rsrp_5g, sysinfo.rsrq_5g,sysinfo.rssi_3g,sysinfo.signal_strength_3g,sysinfo.network_type,sysinfo.simcard_status,sysinfo.sms_unread_count);

	showSystemInfo(cpu_now,1);
	
	show_battery(sysinfo.bat_charge, sysinfo.bat_value,sysinfo.bat_percent);
}
...
function showSystemInfo(cpu_now,force){
	...
	//showtext($("modem_isp_top"), '<% nvram_get_x("",  "operator"); %>');
...
function showSignalInfo(ltesignal,rsrp_4g,rsrq_4g,signal_5g,rsrp_5g,rsrq_5g,rssi_3g,signal_strength_3g,network_type,simcard_status,sms_unread_count) {
	var signal = ltesignal, rsrp = rsrp_4g, rsrq = rsrq_4g,rssi= rssi_3g;

	if(simcard_status == 1){
    if(signal_5g != "" && signal_5g != 0)
        signal = signal_5g;
    else if(signal_strength_3g != "" && signal_strength_3g != 0)
        signal = signal_strength_3g;
	else if (ltesignal != "")
		signal = ltesignal;
		
    if (rsrp_5g != "" && rsrp_5g != 0)
		rsrp = rsrp_5g;
	else if (rsrp_4g != "")
		rsrp = rsrp_4g;
	
	if (rsrq_5g != "" && rsrq_5g != 0)
		rsrq = rsrq_5g;	
	else if (rsrq_4g != "")
		rsrq = rsrq_4g;
    
	$("lte_signal").innerHTML = "<img  src='/bootstrap/img/signal/Signal" + signal +".gif'>";
	if (rsrp != "")
		$("rsrp").innerHTML = rsrp + ' dBm';
	//$("rsrq").innerHTML = rsrq;
	if(network_type != "" && network_type != "UNKNOWN"){
		$("3g4gmode").innerHTML = network_type;	
	}else{
		$("3g4gmode").innerHTML = "";
	}
	
	if(rsrq != "undefined"&& rsrq != ""){
	    
	     $("rsrprssiname").innerHTML = "<#rsrp#>";
	    $("rsrp").innerHTML = rsrp + ' dBm';
	}
    else if(rssi && rssi != "undefined" && rssi != ""){
        $("rsrprssiname").innerHTML = "<#rssi#>";
         $("rsrp").innerHTML = rssi;     
    }
    else {
         $("rsrprssiname").innerHTML = "<#rsrp#>";
    ...
    {
        $("newSMS1").style.display = "";
	    $("newSMS1").innerHTML = "<img  src='/images/" + "SMS2.png'>";
    }
    else
    {
         $("newSMS1").style.display = "none";
    }
}
...
function show_banner(L3){
	...
	bc += '  <tr>\n';
	bc += '    <td width="43%" style="border: 0 none;"><#Ltesignal#></td>\n';
	bc += '    <td style="border: 0 none; min-width: 115px;"><div id="lte_signal"> -- -- -- </div></td>\n';
	bc += '  </tr>\n';
	bc += '  <tr>\n';
	bc += '    <td id="rsrprssiname" style="height: 20px;"><#rsrp#></td>\n';
	bc += '    <td><span id="rsrp"> -- </span></td>\n';
	bc += '  </tr>\n';
	bc += '  <tr>\n';
	bc += '    <td><#3g4gmode#></td>\n';
	bc += '    <td><span id="3g4gmode"> -- </span></td>\n';
	bc += '  </tr>\n';
	bc += '  <tr>\n';
	bc += '    <td><#SI_Uptime#></td>\n';
...
//	bc += '  </tr>\n';
	...
	bc += '  <tr>\n';
    bc += '    <td><#Battery_Level#></td>\n';
    bc += '    <td id="battery_level_top" colspan="2" style="text-align: left;width: 160px;"></td> \n';
    bc += '   </tr>\n';
	
    bc += '  <tr>\n';
    bc += '   <td><#Simcardstatus#></td>\n';
    bc += '   <td id="simcard_status" colspan="2" style="text-align: left;width: 160px;"></td>\n';
...
//     bc +=  '  <tr>\n';
//    bc += '    <td >&nbsp&nbsp&nbsp</td>\n';
//     bc += '    <td >&nbsp&nbsp&nbsp&nbsp</td>\n';
//    bc += '  </tr>\n';	
	...
	bc += '   <td id="newSMS1" class="info-img" style="padding-top: 3px;display:none;"> <img id="newSMS" src="" style="min-width: 115px"/>  </td>';
	bc += '  </tr>\n';
	bc += '</table>\n';
	bc += '</div>\n';
	bc += '</div>\n';
...
tabtitle[0] = new Array("", "<#menu5_11#>"/*, "<#menu5_1_2#>", "<#menu5_1_3#>", "<#menu5_1_4#>", "<#menu5_1_5#>", "<#menu5_1_6#>"*/);
tabtitle[1] = new Array("", "<#menu5_12#>"/*, "<#menu5_12_1#>", "<#menu5_1_2#>", "<#menu5_1_3#>", "<#menu5_1_4#>", "<#menu5_1_5#>", "<#menu5_1_6#>"*/);
tabtitle[2] = new Array("", /*"<#menu5_2_1#>",*/ "<#menu5_2_2#>"/*, "<#menu5_2_3#>", "<#menu5_2_4#>", "<#menu5_2_5#>", "<#menu5_2_6#>"*/);
tabtitle[3] = new Array("","<#menu5_3_1#>", "<#menu5_3_3#>"/*, "<#menu5_3_6#>"*/);//WAN
tabtitle[4] = new Array("", "<#menu5_5_1#>", /*"<#menu5_5_5#>", "<#menu5_5_2#>", */"<#menu5_5_3#>", /*"<#menu5_5_4#>",*/ "<#menu5_5_6#>", "<#menu5_5_7#>", "<#menu5_5_8#>", "<#menu5_5_9#>");
//tabtitle[5] = new Array("", "<#menu5_4_3#>", "<#menu5_4_1#>", "<#menu5_4_2#>", "<#menu5_4_4#>", "<#menu5_4_5#>");
tabtitle[5] = new Array("", "<#menu5_6_12#>", "<#menu5_6_14#>","<#menu5_6_15#>","<#menu5_4_6#>");
tabtitle[6] = new Array("", "<#menu5_6_2#>", "<#menu5_6_16#>","<#menu5_6_3#>", "<#menu5_6_4#>","<#menu5_6_13#>","<#menu5_6_17#>"/*, "<#menu5_6_11#>","<#menu5_6_12#>",*/);
//tabtitle[7] = new Array("", "<#menu5_10_1#>", "<#menu5_10_2#>", "<#menu5_10_3#>");
tabtitle[7] = new Array("", "<#menu5_7_9#>","<#menu5_7_10#>"/*,"<#menu5_7_11#>"*/);
tabtitle[8] = new Array("", "<#menu5_18_7#>");
tabtitle[9] = new Array("", "<#menu5_18_20#>");
tabtitle[10] = new Array("", "<#menu9#>","<#menu9_1#>");
tabtitle[11] = new Array("", "<#menu5_7_2#>"/*, "<#menu5_7_3#>", "<#menu5_7_5#>", "<#menu5_7_6#>", "<#menu5_7_8#>"*/);
tabtitle[12] = new Array("", "<#menu5_10_1#>");
tabtitle[13] = new Array("", "<#menu5_11_1#>", "<#menu5_11_2#>", "<#menu5_11_4#>", "<#menu5_11_3#>");
...
tablink[1] = new Array("", "Advanced_Wireless_Content.asp"/*, "Advanced_Wireless_Content2.asp", "Advanced_WGuest_Content.asp", "Advanced_WMode_Content.asp", "Advanced_ACL_Content.asp", "Advanced_WSecurity_Content.asp", "Advanced_WAdvanced_Content.asp"*/);
tablink[2] = new Array("", "Advanced_LAN_Content.asp"/*, "Advanced_DHCP_Content.asp", "Advanced_VPN_Content.asp"*//*, "Advanced_GWStaticRoute_Content.asp", "Advanced_IPTV_Content.asp", "Advanced_Switch_Content.asp", "Advanced_WOL_Content.asp"*/);
tablink[3] = new Array("", "Advanced_WAN_Content.asp", "Advanced_IPv6_Content.asp"/*,  "Advanced_DDNS_Content.asp"*/);
tablink[4] = new Array("", "Advanced_BasicFirewall_Content.asp", /*"Advanced_Netfilter_Content.asp", "Advanced_URLFilter_Content.asp", */"Advanced_MACFilter_Content.asp", /*"Advanced_Firewall_Content.asp",*/"Advanced_VirtualServer_Content.asp","Advanced_Exposed_Content.asp","Advanced_IPFiltering.asp","Advanced_PortFiltering.asp");//??ÁÅ??//tablink[5] = new Array("", "Advanced_AiDisk_others.asp", "Advanced_AiDisk_samba.asp", "Advanced_AiDisk_ftp.asp", "Advanced_Modem_others.asp", "Advanced_Printer_others.asp");
tablink[5] = new Array("", "Advanced_Settings_Content.asp","Advanced_Settings_Band.asp","Advanced_CellLock_Content.asp","Advanced_MIMEI_Content.asp");
tablink[6] = new Array("", "Advanced_System_Content.asp", "Advanced_System_Adb.asp", "Advanced_FirmwareUpgrade_Content.asp", "Advanced_SettingBackup_Content.asp","Advanced_System_AutoSleep.asp","Advanced_System_timed_reboot.asp"/*, "Advanced_NMS_Content.asp", "Advanced_Settings_Content.asp",*/);
//tablink[7] = new Array("", "Advanced_Tweaks_Content.asp", "Advanced_Scripts_Content.asp", "Advanced_InetDetect_Content.asp");
tablink[7] = new Array("", "Advanced_VPN_Content.asp","Advanced_VPN_PassThrough_Content.asp"/*,"Advanced_VPN_Cert_Content.asp"*/);
tablink[8] = new Array("", "Advanced_DNS_Content.asp"/*, "Main_DHCPStatus_Content.asp", "Main_IPTStatus_Content.asp", "Main_RouteStatus_Content.asp", "Main_CTStatus_Content.asp"*/);
tablink[9] = new Array("", "Advanced_DDNS_Content.asp");
tablink[10] = new Array("", "Advanced_Parent_Control.asp","Advanced_Parent_Control_List.asp");
tablink[11] = new Array("", "Main_LogStatus_Content.asp"/*, "Main_DHCPStatus_Content.asp", "Main_IPTStatus_Content.asp", "Main_RouteStatus_Content.asp", "Main_CTStatus_Content.asp"*/);
tablink[12] = new Array("", "","","");
tablink[13] = new Array("", "smsInbox.asp","smsOutbox.asp","smsNotSendbox.asp","smsSend.asp");
if (found_app_scutclient()){
...
menuL2_title = new Array("", "<#menu5_11#>", "<#menu5_12#>", "<#menu5_2#>", "<#menu5_3#>", "<#menu5_5#>", "<#menu5_4#>", "<#menu5_6#>", /*"<#menu5_9#>", */"<#menu5_7#>","<#menu5_18_7#>","<#menu5_18_20#>","<#menu9#>","<#menu5_8#>");
if (found_app_scutclient()){
...
menuL1_title = new Array("", "<#menu1#>", /*"<#menu3#>", "<#menu2#>", "<#menu6#>",*/ "<#menu4#>", /*"<#menu7#>", */"<#menu11#>","<#menu5#>");
menuL1_link = new Array("", "index.asp", /*"aidisk.asp", "vpnsrv.asp", "vpncli.asp",*/ "Main_TrafficMonitor_realtime.asp", /*"SpeedTest.asp", */"smsInbox.asp","as.asp");
menuL1_icon = new Array("", "icon-home", /*"icon-hdd", "icon-retweet", "icon-globe", "icon-tasks", "icon-random", */"icon-tasks","icon-envelope","icon-wrench");
...
function show_menu(L1, L2, L3){
	...
	if(sw_mode == '3'){
		...
		tabtitle[8].splice(2,4);//log
		tablink[8].splice(2,4);
		...
		menuL2_title[4] = "";
	/*
	for (i=0;i<num_ephy - 1;i++){
		tablink[7][i+3] = "Main_EStatus_Content.asp#"+i.toString();
		if (i>0)
			tabtitle[7][i+3] = "LAN"+i.toString();
	}
	*/
	if(num_ephy<8){
...
function show_footer(){
	...
	//footer_code +='  <span> CP200 </span></br>\n';
...
function show_top_status(){
	...
	showtext($("firmver"), '<% nvram_get_x("",  "fw_ver"); %>');
