...
function getclients(flag_mac,flag_all){
	...
	for(var i = 0; i < ipmonitor.length; ++i){
		...
		clients[j][4] = ipmonitor[i][6]; /*null;	*/		// this is a wireless info->change to type of connection
		clients[j][5] = ipmonitor[i][3];	// TYPE
		clients[j][6] = ipmonitor[i][4];	// host has a HTTP service
		clients[j][7] = "u";
		clients[j][8] = ipmonitor[i][7];
		clients[j][9] = ipmonitor[i][8];
...
//		
	...
	if(typeof list_of_BlockedClient !== 'undefined'){
		for(var i = 0; i < list_of_BlockedClient.length; i++){
			var repeat = 0;
			for(var k = 0; k < j; k++){
				if(clients[k][2] == list_of_BlockedClient[i][0]){
					repeat = 1;
					break;
				}
			}
			if(repeat == 0 && list_of_BlockedClient[i][1] != ""){
				clients[j] = new Array(8);
				clients[j][0] = "";
				clients[j][1] = "";
				clients[j][2] = list_of_BlockedClient[i][0];
				clients[j][3] = null;
				clients[j][4] = list_of_BlockedClient[i][1];
				++j;
			}
		}
	}
